modifier_item_ice_trap_thinker = class({})

function modifier_item_ice_trap_thinker:OnCreated(params)
    if IsServer() then 
        self.team = params.team

        self.efx_index = ParticleManager:CreateParticle("particles/ice_trap/creating.vpcf", PATTACH_WORLDORIGIN, nil)
...
function modifier_item_ice_trap_thinker:OnDestroy()
    if IsServer() then
        local bounce_trap = CreateUnitByName("npc_dota_ice_trap", self:GetParent():GetOrigin(), true, nil, nil, self.team)
